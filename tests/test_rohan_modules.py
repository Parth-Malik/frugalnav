...
def test_soft_trigger_fires_above_tau():
    # A realistic drifting state (several leading cues elevated together) must fire
    # the SOFT trigger before sigma_pos reaches the hard floor -- the whole point of
    # the leading indicators. Regression for the too-sparse original self-test case.
    sch = UncertaintyScheduler()
    drifting = dict(sigma_pos=1.2, sigma_head=0.20, feature_loss=25.0,
                    blur=180.0, imu_bias=0.08, active_features=80)
    U, trig, why, _ = sch.compute(dict(drifting))
    assert trig and why == "soft_U" and U > SchedulerConfig().tau
    # and the refractory window suppresses an immediate re-trigger
    sch.reset_after_fix()
    _, trig2, _, _ = sch.compute(dict(drifting))
    assert not trig2
