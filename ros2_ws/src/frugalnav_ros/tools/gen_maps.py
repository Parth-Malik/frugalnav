...
<sdf version="1.6">
  <world name="{name}">
    ...
    <scene><ambient>0.5 0.52 0.55 1</ambient><background>{bg}</background><shadows>true</shadows>
      <fog><color>{bg}</color><type>linear</type><start>{fog_start}</start><end>{fog_end}</end><density>{fog_density}</density></fog></scene>
...
def build_india():
    ...
    # ArUco tiles: a DENSE line down the flight corridor so the drift-prone estimator gets
    # constant fixes and never runs off, PLUS a coarse grid over the rest of the block so the
    # whole map is marked, not just the flight line, up to the 100 tile budget.
    markers, mid = [], 0
    for mx in range(-96, 97, 4):                          # dense corridor line, every 4 m
        if clear(float(mx), 0.0, 2.0):
            markers.append((mid, float(mx), 0.0)); mid += 1
    for my in range(-84, 85, 24):                         # even, symmetric spread over the block
        for mx in range(-84, 85, 24):
            if mid >= 100:
                break
            if abs(my) < 4:
                continue                                  # corridor is already covered
    ...
    with open(os.path.join(PKG, "worlds", "india.world"), "w") as f:
        f.write(world("india", bodies, (0.46, 0.42, 0.34), bg="0.74 0.77 0.80 1",
                      fog_density=0.002, fog_start=260, fog_end=900))     # clear air, see the whole block
